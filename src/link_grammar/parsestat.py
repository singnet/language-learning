...
def parse_metrics(tokens: list) -> ParseMetrics:
    ...
    if unlinked == 0:
        pm.completely_parsed_ratio = 1.0
        pm.average_parsed_ratio = 1.0
    else:
        pm.average_parsed_ratio = 1.0 - float(unlinked) / float(total)

    if total == unlinked:
        pm.completely_unparsed_ratio = 1.0
...
def parse_quality(test_set: set, ref_set: set) -> ParseQuality:
    ...
    if len_ref > 0:
        ...
        pq.quality = float(len(test_set & ref_set)) / float(len_ref)
